    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="bg-linear-to-r from-blue-950 via-slate-900 to-indigo-950 text-white rounded-3xl p-6 sm:p-8 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl sm:text-2xl font-extrabold">
              Document Vault & DigiLocker Locker
            </h2>
          </div>
          <p className="text-xs sm:text-sm text-blue-200 max-w-xl leading-relaxed">
            Encrypted storage and instant verified access to your official vehicle RCs, driving licences, PUC certificates, and tax receipts.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{isSyncing ? 'Syncing DigiLocker...' : 'Sync with DigiLocker'}</span>
          </button>
        </div>
      </div>
      ...
      {syncNotice && (
        <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-2xl text-xs text-emerald-950 flex items-center gap-2 shadow-xs animate-in fade-in">
          ...
          <span>
            <strong>DigiLocker Sync Complete:</strong> All 5 digital credentials and Smartcard certificates are up-to-date with MoRTH National Register.
          </span>
        </div>
      )}
        ...
        {vaultDocs.map((doc) => (
          ...
          >
            <div>
              ...
              <p className="text-xs font-mono font-bold text-blue-950 mb-2">
                {doc.documentNumber}
              </p>
              <p className="text-[11px] text-slate-500 mb-3">{doc.issuer}</p>

              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-[11px] text-slate-600 space-y-1 mb-4">
                <div className="flex justify-between">
                  <span>Linked Identifier:</span>
                  <strong className="text-slate-800">{doc.linkedIdentifier}</strong>
                </div>
                <div className="flex justify-between">
                  <span>Valid Till:</span>
                  <strong className="text-slate-800">{formatDate(doc.validTill)}</strong>
                </div>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center gap-2">
              <button
                ...
                className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold py-2 rounded-xl transition flex items-center justify-center gap-1.5 cursor-pointer"
              >
      ...
      {/* Document Detailed Preview Modal (Print Ready) */}
      {selectedDoc && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
            ...
            {/* Printable Digital Certificate Format */}
            <div className="p-8 overflow-y-auto space-y-6 text-xs bg-slate-50 font-sans">
              ...
              <div className="text-center border-b-2 border-slate-300 pb-4 bg-white p-6 rounded-2xl shadow-xs border">
                ...
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Issued under the provisions of the Motor Vehicles Act, 1988
                </p>
              </div>
